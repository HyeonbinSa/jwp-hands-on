...
class SynchronizationTest {
    ...
    @Test
    void testSynchronized2() throws InterruptedException {
        var executorService = Executors.newFixedThreadPool(3);
        var synchronizedStatements = new SynchronizedStatements();

        IntStream.range(0, 1000)
                .forEach(count -> executorService.submit(synchronizedStatements::calculate));
        executorService.awaitTermination(500, TimeUnit.MILLISECONDS);

        assertThat(synchronizedStatements.getSum()).isEqualTo(1000);
    }
    ...
    private static final class SynchronizedStatements {
